  ...
  return (
    <div>
      <div className='flex justify-center'>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-1.5 w-[95%] md:w-[60%] lg:w-[36%] rounded-xl border border-neutral-500 px-3 lg:px-6 py-4 m-10 mt-15 lg:m-15 shadow-[0_0_15px_rgba(0,0,0,0.2)]'>
          <h1 className='text-2xl font-bold mb-5'>Sign in</h1>
          <label className='text-xl' htmlFor="email">Email address</label>
          <input onChange={(e)=>setEmail(e.target.value)} value={email} required id='email' className='py-2 lg:py-2.75 bg-gray-300 outline-neutral-500 px-2 rounded-lg placeholder:text-gray-900 border border-neutral-400' type="email" placeholder='Enter email' />
          <label className='text-xl mt-2.5' htmlFor="password">Password</label>
          <input onChange={(e)=>setPassword(e.target.value)} value={password} required id='password' className='py-2 lg:py-2.75 bg-gray-300 outline-neutral-500 px-2 rounded-lg placeholder:text-gray-900 border border-neutral-400 '  type="password" placeholder="Enter password" />
           {
